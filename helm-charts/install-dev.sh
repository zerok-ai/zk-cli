...
if [ -z "$ZK_SCENARIO_MANAGER_VERSION" ] || [ -z "$ZK_AXON_VERSION" ] || [ -z "$ZK_DAEMONSET_VERSION" ] || [ -z "$ZK_GPT_VERSION" ] || [ -z "$ZK_WSP_CLIENT_VERSION" ]  || [ -z "$ZK_OPERATOR_VERSION" ] || [ -z "$ZK_APP_INIT_CONTAINERS_VERSION" ] || [ -z "$ZK_CLOUD_ADDR" ]
then
  echo "Invalid cli arguments. ERR #2"
  exit 1
fi
...
helm repo add zk-app-init-containers https://privatehelm.zerok.ai/zk-client/zk-app-init-containers
...
# install
helm upgrade zk-app-init-containers zk-app-init-containers/zk-app-init-containers --install --create-namespace --namespace zk-client --version $ZK_APP_INIT_CONTAINERS_VERSION
